...
module Search
  class DailyReport
    ...
    def search
      ...
      reports = reports.where('created_at >= ?', from.to_s) if from.present?
      reports = reports.where('created_at <= ?', to.to_s) if to.present?
      reports.order(converted_sort.to_sym => converted_order_by)
    end
